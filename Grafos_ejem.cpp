//
// Created by jcasiano on 14/7/2021.
//
...
int main() {
    ...
    while(temporal<=fin)
    {
        int vecino=grafo->primero(temporal);
        if(maximo==vecino) encontrado=true;
        if(maximo<vecino && vecino<=fin) maximo=vecino;
        while(vecino<grafo->get_N_Vertices()){
            vecino = grafo->siguiente(temporal, vecino);
